...
void *consumer(void *arg) {
	...
	char *line;
	int exit = 0;
	int stat_add_buf[MAX_STRING_LENGTH];
	int stat2_add_buf[ASCII_SIZE];
	char *cptr = NULL;
	char *substr = NULL;
	char *brka = NULL;
	char *linebuffer;
...
//--------------start of cons lock------------------------------------------------------------------------
		for(int i=0; i<MAX_STRING_LENGTH; i++) stat[i] += stat_add_buf[i];
		for(int i=0; i<ASCII_SIZE; i++) stat2[i][0] += stat2_add_buf[i];
		...
		if (line != NULL) {
			len = strlen(line);
			linebuffer = (char*)malloc((len+1) * sizeof(char));
...
//--------------End of cons lock------------------------------------------------------------------------------------
		...
		memset(stat_add_buf, 0, sizeof(stat_add_buf));
		memset(stat2_add_buf, 0, sizeof(stat2_add_buf));
